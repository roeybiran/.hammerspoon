...
local function pushToCell(direction)
	...
	-- Try using selectMenuItem first for the basic direction
	-- local menuItemName = directionMapping[direction]
	-- if menuItemName then
	-- 	local result = hs.application.frontmostApplication():selectMenuItem(menuItemName)
	-- 	if result then
	-- 		return
	-- 	end
	-- end
	...
	for _, cellProperties in pairs(possibleCells) do
		if frontWindowFrame:equals(cellProperties.rect) then
			...
			-- Try selectMenuItem for the target cell (including diagonals)
			local targetMenuItemName = directionMapping[targetCellName]
			print(targetMenuItemName, targetCellName)
			if targetMenuItemName then
				local result = hs.application.frontmostApplication():selectMenuItem(targetMenuItemName)
				if result then
					return
				end
			end
